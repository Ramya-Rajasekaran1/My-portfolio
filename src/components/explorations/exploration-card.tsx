...
export function ExplorationCard({ exploration, index }: ExplorationCardProps) {
  ...
  return (
    <motion.article
      ...
      className="group"
    >
      <div className="bg-[#161122] rounded-3xl border border-white/5 overflow-hidden transition-all duration-300 hover:border-[#F79CBa]/25">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-0">
          ...
          >
            ...
            />
            <div className="absolute inset-0 bg-gradient-to-t from-[#161122] via-transparent to-transparent opacity-80 lg:opacity-40" />
          ...
          <div className="lg:col-span-5 p-8 md:p-10 flex flex-col justify-center gap-6">
            <div>
              <span className="text-[10px] font-mono uppercase tracking-[0.35em] text-[#F79CBa] mb-3 block">
                {exploration.subtitle}
              </span>
              <h3 className="font-serif text-3xl md:text-4xl font-bold text-white leading-tight mb-4 group-hover:text-[#FFC4D4] transition-colors">
                {exploration.title}
              </h3>
              <p className="text-[#CFC5BA] leading-relaxed text-sm md:text-base">
            ...
            <div className="flex flex-wrap gap-2">
              {exploration.tags.map((tag) => (
                <span
                  key={tag}
                  className="text-[10px] font-mono uppercase tracking-wider text-[#CFC5BA]/80 bg-white/5 border border-white/10 rounded-full px-3 py-1"
                >
                  {tag}
                </span>
              ))}
            ...
            <div className="flex flex-wrap gap-3 pt-2">
              <a
                ...
                className="inline-flex items-center gap-2 bg-[#F79CBa] hover:bg-[#FFC4D4] text-[#4A152B] font-bold px-5 py-2.5 rounded-full text-xs uppercase tracking-widest transition-all duration-300"
              >
                Launch experience
