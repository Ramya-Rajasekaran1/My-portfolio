...
export function SpiralGlobe() {
    ...
    const points = useMemo(() => {
        ...
        for (let i = 0; i < count; i++) {
            // Tighter Spiral distribution
            const phi = Math.acos(-1 + (2 * i) / count);
            const theta = 12 * phi; // Tighter spiral factor

            pts.push({
                x: Math.cos(theta) * Math.sin(phi),
                y: Math.sin(theta) * Math.sin(phi),
                z: Math.cos(phi),
                baseOpacity: 0.6 + (i / count) * 0.4 // High base opacity for brightness
            });
        }
        return pts;
    }, []);
    ...
    useEffect(() => {
        let frameId: number;
        const animate = () => {
            timeRef.current += 0.008; // Slightly slower, more elegant
            setTick(prev => prev + 1);
